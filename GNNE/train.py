...
def prepare_data(graphs, args, test_graphs=None, max_nodes=0):
    ...
    if test_graphs is None:
        ...
        train_idx = int(len(graphs) * args.train_ratio)
        train_graphs = graphs[:train_idx]
        val_graphs = graphs[train_idx:] #graph
    print(
        "Num training graphs: ",
        len(train_graphs),
...
def train_node_classifier_multigraph(G_list, labels, model, args, writer=None):
    ...
    for epoch in range(args.num_epochs):
        ...
        loss.backward()
        # nn.utils.clip_grad_norm(model.parameters(), args.clip)  #这个语句被遗弃了
        torch.nn.utils.clip_grad_norm_(model.parameters(), args.clip)
...
def benchmark_task_val(args, writer=None, feat="node-label", cross_val=None):# 另外加的cross_val=None
    all_vals = []
    graphs = io_utils.read_graphfile(
        args.datadir, args.bmname, max_nodes=args.max_nodes
    )
